...
func NewDriver(a *Adaptor, topic string) *Driver {
	m := &Driver{
		name:       framework.DefaultName("MQTT"),
		topic:      topic,
		connection: a,
		Eventer:    framework.NewEventer(),
		Commander:  framework.NewCommander(),
	}
...
func (m *Driver) Connection() framework.Connection {
	return m.connection
}
