...
func NewJSONRobot(luakit *LuaKit) *JSONLuaKit {
	jsonRobot := &JSONLuaKit{
		Name:        luakit.Name,
		Commands:    []string{},
	...
	for command := range luakit.Commands() {
		jsonRobot.Commands = append(jsonRobot.Commands, command)
	}

	luakit.Devices().Each(func(device Device) {
		jsonDevice := NewJSONDevice(device)
		jsonRobot.Connections = append(jsonRobot.Connections, NewJSONConnection(luakit.Connection(jsonDevice.Connection)))
...
func (r *LuaKits) Len() int {
	return len(*r)
}

// Start calls the Start method of each Robot in the collection
func (r *LuaKits) Start(args ...interface{}) (err error) {
	autoRun := true
	if args[0] != nil {
		autoRun = args[0].(bool)
	}
	for _, luakit := range *r {
		if rerr := luakit.Start(autoRun); rerr != nil {
...
func (r *LuaKits) Stop() (err error) {
	for _, luakit := range *r {
		if rerr := luakit.Stop(); rerr != nil {
			err = multierror.Append(err, rerr)
			return
		}
	}
...
func (r *LuaKits) Each(f func(kit *LuaKit)) {
	for _, robot := range *r {
		f(robot)
	}
}
...
//
func NewRobot(v ...interface{}) *LuaKit {
	r := &LuaKit{
...
func (r *LuaKit) Start(args ...interface{}) (err error) {
...
// does not exist.
func (r *LuaKit) Connection(name string) Connection {
	if r == nil {
		return nil
	}
