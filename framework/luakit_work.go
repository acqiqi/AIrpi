...
func (rw *LuaKitWork) TickCount() int {
	return rw.tickCount
}

// Duration returns the timeout until an After fires or the period of an Every
func (rw *LuaKitWork) Duration() time.Duration {
	return rw.duration
}

func (rw *LuaKitWork) String() string {
	format := `ID: %s
Kind: %s
TickCount: %d
`
...
func (r *LuaKit) WorkRegistry() *RobotWorkRegistry {
	return r.workRegistry
}

// Every calls the given function for every tick of the provided duration.
func (r *LuaKit) Every(ctx context.Context, d time.Duration, f func()) *LuaKitWork {
...
func (r *LuaKit) After(ctx context.Context, d time.Duration, f func()) *LuaKitWork {
...
// map.
func (rwr *RobotWorkRegistry) Get(id uuid.UUID) *LuaKitWork {
	rwr.Lock()
	defer rwr.Unlock()
...
func (rwr *RobotWorkRegistry) registerAfter(ctx context.Context, d time.Duration, f func()) *LuaKitWork {
	rwr.Lock()
	defer rwr.Unlock()
...
func (rwr *RobotWorkRegistry) registerEvery(ctx context.Context, d time.Duration, f func()) *LuaKitWork {
	rwr.Lock()
	defer rwr.Unlock()
